...
class TestCSVDataset(unittest.TestCase):
    def setUp(self) -> None:
        self.dataset = CSVDataset(text_field=Field(), file_name='../test_data/train.csv')
    ...
    def test_data_read_altered_col_names(self):
        train_data = pd.read_csv('../test_data/different_headers.csv')
        self.dataset.file_name = '../test_data/different_headers.csv'
    ...
    def test_data_read_multiple_cols(self):
        train_data = pd.read_csv('../test_data/multicol_train.csv')
        self.dataset.file_name = '../test_data/multicol_train.csv'
        output = self.dataset.load()
        self.assertEqual(len(output), len(train_data))
        self.assertTrue(hasattr(output[0], 'text'))
        self.assertTrue(hasattr(output[0], 'label'))
